...
public abstract class CategoryMapper {
    ...
    protected void mapCategoryResponse(Category request, @MappingTarget CategoryResponse response) {
        ...
        List<Restaurant> restaurants = request.getMenuRestaurants().stream().map(MenuRestaurant::getRestaurant).toList();
        response.setRestaurants(restaurants.stream().map(restaurantMapper::toRestaurantCompact).toList());
